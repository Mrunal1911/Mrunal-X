...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "MrunalX"
...
async def unload(event):
    ...
    try:
        ...
        qwe = await eor(event, f"MrunalX Has Successfully unloaded {shortname}")
    except Exception as e:
        await qwe.edit(
            "MrunalX has Successfully unloaded {shortname}\n{}".format(shortname, str(e))
        )
...
async def load(event):
    ...
    try:
        ...
        await qwe.edit(
            f"MrunalX could not load {shortname} because of the following error.\n{str(e)}"
        )
